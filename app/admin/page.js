...
function SecaoCupons() {
  ...
  function importarBulk() {
    ...
    if (!novos.length) {
      setMsg("Nenhum cupom válido (cada linha precisa ter pelo menos o código).");
      return;
    }
    persistir([...lista, ...novos]);
    setBulk("");
  }
      ...
      {carregando ? (
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="border-t border-cc-line pt-4">
            <p className="mb-2 text-sm font-semibold text-cc-ink">Adicionar vários de uma vez</p>
            <p className="mb-2 text-xs text-cc-muted">
              Um cupom por linha, no formato:{" "}
              <span className="cc-mono">plataforma | código | descrição | validade</span>. Só a
              plataforma e o código são obrigatórios. Ex:{" "}
              <span className="cc-mono">shopee | CUPOM10 | 10% OFF | 30/06</span>
            </p>
            <textarea
              value={bulk}
              ...
              className="w-full border border-cc-line p-3 font-mono text-xs outline-none focus:border-cc-yellow"
            />
            <button
              onClick={importarBulk}
              disabled={salvando}
              className="mt-2 border border-cc-ink px-4 py-2 text-sm font-semibold text-cc-ink transition hover:bg-cc-cream active:translate-y-px disabled:opacity-60"
            >
              Importar lista
            </button>
          </div>
