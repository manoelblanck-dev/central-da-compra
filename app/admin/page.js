...
function SecaoCupons() {
  ...
  function adicionar() {
    ...
    const nova = [
      ...lista.filter((c) => c.plataforma !== novo.plataforma), // 1 cupom por plataforma
      { ...novo, codigo: codigo.toUpperCase() },
    ];
    persistir(nova);
  ...
  return (
    <div className="mt-6 border border-cc-line bg-white p-5">
      <h2 className="cc-mono text-xl text-cc-ink">🎟️ Cupons ativos (por plataforma)</h2>
      <p className="mt-1 text-xs text-cc-muted">
        Cadastre 1 cupom por plataforma. Ele aparece automaticamente em todos os produtos
        daquela plataforma e numa faixa na home. Para trocar, é só adicionar de novo.
      </p>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="bg-cc-yellow px-5 py-2.5 text-sm font-bold text-cc-ink transition hover:bg-cc-yellow-dark active:translate-y-px disabled:opacity-60"
            >
              {salvando ? "Salvando..." : "Adicionar / atualizar cupom"}
            </button>
            {msg ? <span className="text-sm text-cc-ink">{msg}</span> : null}
          </div>
        </div>
