...
// Aberturas que chamam a atenção. Variam a cada disparo pra o canal não ficar
// robótico (repetindo sempre a mesma frase).
const ABERTURAS = [
...
// caracteres para legenda de foto. Foco em conversão: abertura chamativa,
// destaque do desconto/economia, urgência honesta e empurrão pro botão.
function montarLegenda(produto) {
  ...
  const linhas = [abertura, "", `🛍️ <b>${nome}</b>`, ""];
  ...
  if (preco) {
    ...
    if (desc) {
      linhas.push(
        `🔥 ${desc}% OFF${economia ? ` — você economiza ${escaparHtml(economia)}` : ""}`
      );
    }
  } else {
  ...
  linhas.push("👇 Toque em <b>Ver oferta</b>");
